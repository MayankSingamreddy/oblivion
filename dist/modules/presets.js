export class PresetManager {
    ...
    getAllPresets() {
        const builtin = Object.entries(this.builtinPresets).map(([host, preset]) => this.createPresetMetadata(host, preset, 'built-in'));
        const custom = Object.entries(this.customPresets).map(([host, preset]) => this.createPresetMetadata(host, preset, 'custom'));
        return { builtin, custom };
    }
    createPresetMetadata(host, preset, type) {
        return {
            id: host,
            name: preset.name,
    ...
    generateTags(preset) {
        ...
        for (const rule of preset.rules) {
            if (rule.selector.includes('ad'))
                tags.add('ads');
            if (rule.selector.includes('sidebar'))
                tags.add('sidebar');
            if (rule.selector.includes('recommend'))
                tags.add('recommendations');
            if (rule.selector.includes('trend'))
                tags.add('trending');
            if (rule.selector.includes('comment'))
                tags.add('comments');
            if (rule.action === 'hide')
                tags.add('hiding');
            if (rule.action === 'blank')
                tags.add('blanking');
        ...
        try {
            ...
            for (const rule of preset.rules) {
                try {
                    ...
                    errors.push(`Failed to apply rule ${rule.id}: ${error.message}`);
                }
            }
            ...
            if (preset.themes) {
                try {
                    ...
                    errors.push(`Failed to apply themes: ${error.message}`);
                }
            }
            return { success: applied > 0, applied, errors };
        }
        catch (error) {
            return { success: false, applied: 0, errors: [error.message] };
        }
    }
    // Create custom preset from current page configuration
    async createCustomPreset(host, name, description = '') {
        try {
    ...
    // Import all presets
    async importAllPresets(jsonData) {
        try {
            const importData = JSON.parse(jsonData);
            ...
            if (importData.customPresets) {
                for (const [host, preset] of Object.entries(importData.customPresets)) {
                    try {
                        ...
                        result.errors.push(`Failed to import preset for ${host}: ${error.message}`);
                    }
                }
            }
    ...
    searchPresets(query) {
        ...
        return combined.filter(preset => preset.name.toLowerCase().includes(lowercaseQuery) ||
    ...
    getRecommendationsForHost(host) {
        const baseDomain = this.extractBaseDomain(host);
    ...
    getPopularPresets() {
        ...
        return popular
            ...
            .map(preset => this.createPresetMetadata(Object.keys(this.builtinPresets).find(key => this.builtinPresets[key] === preset), preset, 'built-in'));
    }
