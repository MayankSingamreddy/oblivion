// Local/Remote LLM adapter for natural language to CSS selector conversion
import { StructuralContext, LLMResponse, AIModelConfig, Settings } from '../types/types';
...
export class NLAgent {
  ...
  private readonly requestCooldown = 2000; // 2 seconds between requests
  ...
  async generateSelectors(prompt: string): Promise<LLMResponse> {
    // Rate limiting
    const now = Date.now();
  ...
  private async generateWithLocalModel(prompt: string): Promise<LLMResponse> {
  ...
  private currentPromiseHandlers: { resolve: Function; reject: Function } | null = null;

  private handleGenerationComplete(response: string): void {
  ...
  private async generateWithRemoteModel(prompt: string): Promise<LLMResponse> {
    if (!this.settings?.apiKey) {
  ...
  private extractPageContext(): StructuralContext {
  ...
  private extractLandmarks(): Array<{ tag: string; role?: string; classes: string[]; bounds: DOMRect }> {
    const landmarks = [];
  ...
  private extractInteractiveElements(): Array<{ tag: string; type?: string; role?: string; bounds: DOMRect }> {
    const interactive = [];
  ...
  private extractContentAreas(): Array<{ selector: string; bounds: DOMRect; textLength: number }> {
    const contentAreas = [];
  ...
  private buildLocalModelPrompt(userPrompt: string, context: StructuralContext): string {
...
- Confidence 0.0-1.0`;
  ...
  private buildRemoteModelPrompt(userPrompt: string, context: StructuralContext): string {
...
Focus on accuracy over quantity.`;
  ...
  private parseModelResponse(response: string): LLMResponse {
    try {
  ...
  async setupLocalModel(modelId: string, onProgress?: (progress: { text: string; progress: number }) => void): Promise<boolean> {
  ...
  private onModelLoadProgress?: (progress: { text: string; progress: number }) => void;
...
export class PatternFallback {
  private readonly patterns = new Map<RegExp, { selectors: string[]; description: string }>([
    [
  ...
  generateSelectors(prompt: string): LLMResponse {
    const selectors: Array<{ selector: string; description: string; confidence: number }> = [];
