// Theme capsule system with design tokens and ShadowRoot isolation
...
export class ThemeCapsule {
  ...
  private themeStyleTag: HTMLStyleElement | null = null;
  private uiStyleTag: HTMLStyleElement | null = null;
  ...
  private createThemeCapsule(): void {
  ...
  private applyThemeTokens(): void {
    if (!this.themeStyleTag || !this.currentTheme) return;
  ...
  private generateShadowCSS(tokens: ThemeTokens): string {
    return `
      :host {
  ...
  private applyPageLevelTheme(tokens: ThemeTokens): void {
  ...
  private generatePageCSS(tokens: ThemeTokens): string {
  ...
  private tokensToCSS(tokens: ThemeTokens): string {
  ...
  private async loadHostTheme(): Promise<void> {
    try {
  ...
  async applyTheme(tokens: Partial<ThemeTokens>): Promise<boolean> {
    try {
  ...
  createFloatingUI(content: string, position: { x: number; y: number }): HTMLElement {
  ...
  private handleSystemThemeChange(isDark: boolean): void {
  ...
  private getDarkThemeTokens(): ThemeTokens {
    return {
      '--obv-bg': '#1f2937',
      '--obv-accent': '#3b82f6',
  ...
  private setupThemePickerEvents(picker: HTMLElement): void {
    const bgInput = picker.querySelector('#bg-color') as HTMLInputElement;
    const textInput = picker.querySelector('#text-color') as HTMLInputElement;
    const accentInput = picker.querySelector('#accent-color') as HTMLInputElement;
    const applyBtn = picker.querySelector('#apply-theme') as HTMLButtonElement;
    const resetBtn = picker.querySelector('#reset-theme') as HTMLButtonElement;
