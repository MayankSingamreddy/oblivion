// Selection overlay manager with transparent interaction blocking
...
export class OverlayManager {
  ...
  private updateHighlight(element: Element): void {
    ...
    const highlight = this.createHighlightElement(
      ...
      rect.width,
      rect.height,
      this.ruleEngine.isInteractiveElement(element)
    );
  ...
  private createHighlightElement(x: number, y: number, width: number, height: number, isInteractive: boolean): HTMLElement {
  ...
  private showElementInfo(element: Element, rect: DOMRect): void {
  ...
  private createRuleFromSelection(element: Element, useParent: boolean = false, actionType: 'hide' | 'blank' | 'replace' = 'hide'): void {
  ...
  private showInteractiveWarning(element: Element): boolean {
    ...
    return new Promise<boolean>((resolve) => {
      ...
      const confirmBtn = warning.querySelector('#confirm-btn') as HTMLButtonElement;
      const cancelBtn = warning.querySelector('#cancel-btn') as HTMLButtonElement;
    ...
    }) as any; // Type assertion to work around Promise return
  }
  ...
  private isValidSelectionTarget(element: Element): boolean {
  ...
  private notifyRuleCreated(rule: Rule): void {
  ...
  createRuleFromSelector(selector: string, action: 'hide' | 'blank' | 'replace' = 'hide'): boolean {
