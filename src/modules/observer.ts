// Auto-application system with document_start CSS and MutationObserver
...
export class DOMObserver {
  ...
  private pendingMutations = new Set<Element>();
  private debounceTimer: number | null = null;
  private criticalStylesInjected = false;
      ...
      if (mutation.type === 'childList') {
        ...
        for (const node of Array.from(mutation.addedNodes)) {
          if (node.nodeType === Node.ELEMENT_NODE) {
            ...
            if (this.isOblivionElement(element)) continue;
  ...
  private debouncedApplyRules(): void {
    if (this.debounceTimer) {
      clearTimeout(this.debounceTimer);
    }
  ...
  private async processPendingMutations(): Promise<void> {
    ...
    try {
      ...
      for (const rule of rules) {
        if (!this.ruleEngine.validateRule(rule)) continue;
  ...
  private async applyInitialRules(): Promise<void> {
    try {
      const rules = await this.storage.loadHostRules(this.host);
  ...
  watchForSelector(selector: string, callback: (element: Element) => void): () => void {
    ...
    const selectorCallback = (mutations: MutationRecord[]) => {
      for (const mutation of mutations) {
        if (mutation.type === 'childList') {
          for (const node of Array.from(mutation.addedNodes)) {
            if (node.nodeType === Node.ELEMENT_NODE) {
  ...
  private isOblivionElement(element: Element): boolean {
  ...
  async forceReapplyRules(): Promise<number> {
    try {
      const rules = await this.storage.loadHostRules(this.host);
      let totalApplied = 0;
  ...
  private static instance: DOMObserver | null = null;
...
export class FrameObserver {
  private frameObservers = new Map<HTMLIFrameElement, DOMObserver>();
  private ruleEngine: RuleEngine;
  private storage: StorageManager;
  ...
  private initializeFrameWatching(): void {
    ...
    for (const iframe of Array.from(existingIframes)) {
      this.setupFrameObserver(iframe as HTMLIFrameElement);
    }
  }

  private setupFrameObserver(iframe: HTMLIFrameElement): void {
    try {
