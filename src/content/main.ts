// Main content script orchestrator for Oblivion extension
import { Message, MessageResponse, PageInfo, Rule, Settings } from '../types/types';
...
class OblivionContentScript {
  ...
  private themeCapsule: ThemeCapsule;
  private nlAgent: NLAgent;
  private presetManager: PresetManager;
  private patternFallback: PatternFallback;
  ...
  private async applyRule(rule: Rule): Promise<MessageResponse> {
    try {
  ...
  private async removeRule(ruleId: string): Promise<MessageResponse> {
    try {
  ...
  private async handleAIRequest(prompt: string): Promise<MessageResponse> {
    try {
  ...
  private async resetSite(temporary: boolean = true): Promise<MessageResponse> {
    try {
      if (temporary) {
  ...
  private async saveCurrentConfig(): Promise<MessageResponse> {
    try {
      const appliedRules = this.ruleEngine.getAppliedRules();
  ...
  private async loadPreset(presetId: string): Promise<MessageResponse> {
    try {
  ...
  private async exportConfig(): Promise<MessageResponse> {
    try {
  ...
  private async importConfig(data: string): Promise<MessageResponse> {
    try {
  ...
  private async applyPersistedRules(): Promise<void> {
    try {
      const rules = await this.storage.loadHostRules(this.host);
  ...
  private async applyPersistedThemes(): Promise<void> {
    try {
      const themes = await this.storage.loadHostThemes(this.host);
...
let oblivionInstance: OblivionContentScript;
